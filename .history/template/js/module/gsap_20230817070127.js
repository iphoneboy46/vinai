export default function GSAPJS() {
  ...
  let homeReal = document.querySelector(".home-real");
  if(homeReal){
    
    let sections = gsap.utils.toArray(".real-item");

    gsap.to(sections, {
      xPercent: (-100 * ((sections.length - 1) / 1)),
      transform: "translateY((-100 * ((sections.length - 1) / 1))",
